...
import { calendarApi, getDailyPL } from '@/lib/storage';
...
export default function CalendarView() {
  ...
  const { data, refresh } = useAsyncData(async () => {
    ...
    const dayData: { date: string; day: number; event: any; hasSales: boolean }[] = [];
    for (let i = 0; i < daysInMonth; i++) {
      const d = `${year}-${String(month + 1).padStart(2, '0')}-${String(i + 1).padStart(2, '0')}`;
      const event = events.find((e: any) => e.date === d);
      const pl = await getDailyPL(d);
      dayData.push({ date: d, day: i + 1, event, hasSales: pl.totalSales > 0 });
    }
    return { events, days: dayData };
  }, [year, month]);
  ...
  const handleAddEvent = async () => {
    ...
    await calendarApi.create({ date: selectedDate, event_type: eventType, event_name: eventName, notes: '' });
        ...
        <Card>
          <CardContent className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div><Label className="text-xs">Event Name</Label><Input value={eventName} onChange={e => setEventName(e.target.value)} placeholder="e.g. Diwali" /></div>
              <div>
            ...
            {events.filter((e: any) => e.date === selectedDate).map((ev: any) => (
